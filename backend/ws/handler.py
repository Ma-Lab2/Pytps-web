...
class WebSocketManager:
    ...
    async def _handle_set_cursor(self, ws, sid, data):
        ...
        if not session:
            return
        session.spec_cursor = data.get("energy", 0)
        # Just send cursor update, no full re-analysis needed
        await self._send(ws, {"type": "cursor_update", "energy": session.spec_cursor})
